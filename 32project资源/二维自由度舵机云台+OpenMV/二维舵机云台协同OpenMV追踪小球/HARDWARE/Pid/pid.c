 ...
 * @LastEditTime: 2023-07-29 18:23:21
 * @Description:
 */
...
_pid pid_servo_X, pid_servo_Y; // 舵机转向环  X轴  Y轴
...
void PID_param_init(void)
{
    ...
    /*角度环pid参数*/
    pid_angle.target_val = 0.0; // 角度环期望  0度
    pid_angle.actual_val = 0.0;
    pid_angle.err = 0.0;
    pid_angle.err_last = 0.0;
    pid_angle.integral = 0.0;

    pid_angle.Kp = 0.0; // 注意极性
    pid_angle.Ki = 0.0;
    pid_angle.Kd = 0.0;

    /*舵机转向环  X轴 pid参数*/     // 摄像头 中心点坐标为 160 120
    pid_servo_X.target_val = 160.0; // 角度环期望
    pid_servo_X.actual_val = 0.0;
    pid_servo_X.err = 0.0;
    pid_servo_X.err_last = 0.0;
    pid_servo_X.integral = 0.0;

    pid_servo_X.Kp = 0.10; // 注意极性
    pid_servo_X.Ki = 0.0;
    pid_servo_X.Kd = 0.0;

    /*舵机转向环  Y轴 pid参数*/
    pid_servo_Y.target_val = 120.0; // 角度环期望
    pid_servo_Y.actual_val = 0.0;
    pid_servo_Y.err = 0.0;
    pid_servo_Y.err_last = 0.0;
    pid_servo_Y.integral = 0.0;

    pid_servo_Y.Kp = 0.1; // 注意极性
    pid_servo_Y.Ki = 0.0;
    pid_servo_Y.Kd = 0.0;
}
...
float Angle_pid_realize(_pid *pid, float actual_val)
{
    ...
    else if (pid->integral <= -1000) // 积分限幅
    {
        pid->integral = -1000;
    }
...
/**
 * @brief  舵机X轴角度环pid输出函数    X轴 160  Y轴 120
 * @param  actual_val:无
 *	@note 	无
 * @retval 通过PID计算后的输出
 */
float Servo_X_pid_control(void)
{
    ...
    X = ((OpenMv_data1 << 8) | OpenMv_data2); // 实际值 OpenMV传过来的X轴坐标

    if (X > 300)
    {
        X = 300;
    }
    Expect_Pwm = Angle_pid_realize(&pid_servo_X, X); // 进行 PID 计算

    return Expect_Pwm;
}

/**
 * @brief  舵机Y轴角度环pid输出函数    X轴 160  Y轴 120
 * @param  actual_val:无
 *	@note 	无
 * @retval 通过PID计算后的输出
 */
float Servo_Y_pid_control(void)
{
    ...
    float Y;
    Y = OpenMv_data3;                                // 实际值 OpenMV传过来的Y轴坐标
    Expect_Pwm = Angle_pid_realize(&pid_servo_Y, Y); // 进行 PID 计算
