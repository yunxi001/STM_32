...
uint8_t MPU9250_Init(void)
{
    ...
    MPU_Write_Byte(MPU9250_ADDR,MPU_INT_EN_REG,0X00);   //关闭所有中断
	  MPU_Write_Byte(MPU9250_ADDR,MPU_USER_CTRL_REG,0X00);//I2C主模式关闭(I2C主模式用于与磁力计通信)
...
uint8_t MPU_Get_Gyroscope(void)
{
	...
	if(res==0)
	{
		mpu9250_data->gx=((uint16_t)buf[0]<<8)|buf[1];          //此处得值有正负号，应用有符号接收值
		mpu9250_data->gy=((uint16_t)buf[2]<<8)|buf[3];  
		mpu9250_data->gz=((uint16_t)buf[4]<<8)|buf[5];
	} 	
    return res;
}
...
uint8_t MPU_Get_Accelerometer(void)
{
	...
	if(res==0)
	{
		mpu9250_data->ax=((uint16_t)buf[0]<<8)|buf[1];  
		mpu9250_data->ay=((uint16_t)buf[2]<<8)|buf[3];  
		mpu9250_data->az=((uint16_t)buf[4]<<8)|buf[5];
	} 	
    return res;
}
...
uint8_t MPU_Get_Magnetometer(void)
{
	...
	if(res==0)
	{
		mpu9250_data->mx=((uint16_t)buf[1]<<8)|buf[0];  
		mpu9250_data->my=((uint16_t)buf[3]<<8)|buf[2];  
		mpu9250_data->mz=((uint16_t)buf[5]<<8)|buf[4];
	} 	
